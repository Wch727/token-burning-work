...
- ...逐层递推...
- conv5_3: RF = 212 + (3-1)×16 = 244
- pool5: RF = 244 + (2-1)×16 = 260

因此，VGG-16的全连接层看到的输入感受野为260×260，大于输入图像224×224的尺寸——这意味着全连接层隐式地看到了整个图像的上下文信息，这解释了为什么VGG的全局特征分类能力较强。然而，正是这些全连接层的巨大参数量（fc6层：7×7×512×4096 = 102,760,448个参数）构成了VGG的主要瓶颈。
...
GoogLeNet在ILSVRC 2014上的Top-5错误率为6.67%（使用多尺度裁剪和融合）。如果使用单裁剪评估，Top-5错误率约为9.0%，与VGG-19相当。值得注意的是，GoogLeNet的Top-1错误率（约31.3%，多裁剪）优于VGG-16的28.1%——等等，这个数据有误。实际上，GoogLeNet的Top-1错误率在多裁剪评估下约为31.3%，而VGG-16的单裁剪Top-1为28.1%（多裁剪约为26.0%）。VGG-16的多裁剪精度实际上优于GoogLeNet。GoogLeNet的优势主要体现在：（1）参数量仅为VGG-16的4.8%；（2）FLOPs仅为VGG-16的12%；（3）更深的网络结构（22层 vs. 16层）提供了更强的表征能力潜力。如果使用相同的训练时间和计算预算增加VGG的深度，VGG的精度可以进一步提升——但参数效率的劣势无法弥补。
