...
### REPLACED 6.2.1 图像块（Patch）的数学表示
...
这里 $\mathbf{z}_0 \in \mathbb{R}^{N \times D}$ 是Transformer编码器的输入序列，$\mathbf{E}_{\text{pos}} \in \mathbb{R}^{N \times D}$ 为位置编码（详见第2节），分号表示序列维度上的拼接操作（concatenation along the sequence dimension）。
...
### REPLACED 6.2.4 纯Transformer在CV上的可行性证明
...
### REPLACED 6.2.6 ViT与CNN的系统性对比分析
...
### REPLACED 6.7.2 反向去噪过程：Score Matching与DDPM
...
### REPLACED 6.7.4 条件生成：Classifier Guidance与Classifier-Free Guidance
...
### REPLACED 6.7.5 Score Matching与随机微分方程（SDE）视角
...
### REPLACED 6.8.1 DALL-E系列：自回归与扩散的融合
...
### REPLACED 6.8.2 Stable Diffusion与潜在扩散模型
...
### REPLACED 6.8.3 Midjourney的技术路线
...
### REPLACED 6.8.4 Google Imagen：级联扩散与T5语言理解
...
### REPLACED 6.8.5 ControlNet：精确控制的条件扩散
...
### REPLACED 6.9.1 TimeSformer：时空注意力的可分分解
...
### REPLACED 6.9.2 VideoMAE：视频领域的掩码自编码器
...
### REPLACED 6.11.1 Inception Score（IS）
...
### REPLACED 6.11.2 Fréchet Inception Distance（FID）
...
### REPLACED 6.11.3 Precision与Recall
...
### REPLACED 6.11.4 Sliced Fréchet Inception Distance（sFID）
...
### REPLACED 6.11.6 生成模型的系统比较：扩散模型、GAN与自回归模型
