...
### 6.2节 反向去噪过程：Score Matching与DDPM
...
### 6.4节 条件生成：Classifier Guidance与Classifier-Free Guidance
...
### 6.5节 Score Matching与随机微分方程（SDE）视角
...
### 6.9节 Google Imagen：级联扩散与T5语言理解
...
2. **Tubelet连续掩码（Tubelet-wise Masking）：** 在时间维度上对tubelet进行整体掩码——即同一空间位置的所有时间帧的tubelet被一起掩码或保留。这种策略迫使模型学习跨时间的运动信息，因为时间上的变化提供了关于物体运动的宝贵线索。
...
### 8.2节 Fréchet Inception Distance（FID）
...
### 8.4节 Sliced Fréchet Inception Distance（sFID）
