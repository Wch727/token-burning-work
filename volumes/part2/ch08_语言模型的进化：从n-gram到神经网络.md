...
**概率模型的历史脉络。** 语言模型的思想根源可以追溯到香农1948年的《通信的数学理论》，其中香农提出了利用 $n$-元语法近似自然语言概率分布的初步框架。然而，真正将 $n$-gram模型系统化为实用工具的是Jelinek等人（1970s）在IBM speech group的工作——他们为连续语音识别开发了基于马尔可夫假设的统计语言模型，使用互信息和大词汇量的trigram来约束识别搜索空间。与此同时，Good-Turing（1953）和Katz（1987）的平滑技术为数据稀疏性提供了系统性解决方案，使 $n$-gram模型在受限词表上实现了可用的概率估计。

信息论为语言模型评估提供了严格的数学框架。香农熵和交叉熵将"不确定性"和"意外度（surprisal）"精确量化，而困惑度（Perplexity）作为交叉熵的指数变换，提供了与人类直觉直接对应的评估指标——困惑度可以被理解为"每一步的等效分支因子"。这一指标至今仍是语言模型的黄金标准。
...
神经机器翻译（NMT）的出现将语言模型推向了新的高度。Sutskever等人的Seq2Seq架构（2014，NIPS）首次使用端到端神经网络完成翻译任务，而Bahdanau注意力机制（2015，ICLR；2014年arXiv preprint）从根本上解决了固定长度上下文向量的瓶颈问题。注意力机制不仅提升了翻译质量，更重要的是引入了一种全新的计算范式——动态路由（Dynamic Routing）：模型根据输入动态地选择计算路径，而非使用固定的计算图。
...
对于零计数（$c = 0$），Good-Turing 将总概率质量 $q_{\text{GT}} \approx N_1/N$（出现一次的事件占总事件的比例）均匀分配给所有 $N_0$ 个零频 n-gram 类型。注意 $c^*/N$ 给出的是**类型（type）概率质量**，即某个计数为 $c$ 的 n-gram 在全体 n-gram 类型上的份额，**并非**语言模型条件概率 $P(w\mid h)$。

$$q_{\text{GT}}(w,h) = \frac{c^*(c(h,w))}{N} = \frac{(c+1) N_{c+1}}{N_c \cdot N}$$

要得到条件分布 $P(w\mid h)$，必须在固定历史 $h$ 上对 $w$ 归一化，并配合回退/插值（backoff / interpolation）处理未见历史：例如

$$P_{\text{GT}}(w \mid h) \propto q_{\text{GT}}(w,h) \quad\text{（对固定 $h$ 关于 $w$ 归一化；未见 $h$ 则回退到低阶模型）}$$

其中 $q_{\text{GT}}$ 是类型概率质量，$N_0$ 是零频 n-gram 类型数。其直观含义是：每个零频 n-gram 获得的**质量**正比于“罕见事件（仅出现一次）在语料中的占比”，反比于零频类型总数；条件概率则还要做历史归一化。在实践中，$N_0$ 通常未知（穷举所有零频 n-gram 不可行），需通过 Good-Turing 频率定律估计。

对于高频 n-gram（$c$ 较大），$N_{c+1}/N_c \to 1$，因此 $c^*\approx c$，类型质量趋近 MLE 份额；对于低频乃至零频 n-gram，$c^*$ 平滑地分配质量，避免零概率。真正的 $P(w\mid h)$ 仍须经按历史归一化或与低阶模型回退/插值后才可用。
...
当 $c(h, w) = 0$ 时，$P_{\text{disc}}(w \mid h) = 0$；当 $c(h, w) = 1$ 且 $d = 0.75$ 时，分子为 $\max(1-0.75, 0) = 0.25$，并非归零——折扣只回收 $d$ 对应的概率质量，其余仍留给该词，零计数词则通过延续概率获得再分配份额。
...
等号成立当且仅当 $Q = P$ 几乎处处成立（以 $P$ 为测度）。这意味着：最优编码策略必须与真实分布完全一致，任何偏差都会导致编码效率下降。在语言模型的语境中，$H(P, Q)$ 衡量的是：按照真实语言分布 $P$ 采样文本时，用模型 $Q$ 进行预测所需的*平均意外度（surprisal）*。如果 $Q$ 与 $P$ 完全吻合，交叉熵等于真实熵；否则，交叉熵严格大于熵，差值为KL散度 $D_{\text{KL}}(P \parallel Q)$：
...
**困惑度的物理意义：加权平均分支因子。** 困惑度最深刻的直觉是：它可以被理解为模型在每一步预测时的*加权平均分支因子（Weighted Average Branching Factor）*。具体而言，考虑一个虚构的"决策过程"：在每一步，模型面对 $b$ 个等概率的选择，因此每一步的意外度（surprisal）为 $\log b$。困惑度问的问题是："在每一步，模型认为有多少个等价的选择？"

如果 $\text{PPL} = 100$（使用底数2），则模型在每一步的平均意外度为 $\log_2 100 \approx 6.64$ 比特。这意味着模型的不确定性等同于从 $2^{6.64} \approx 100$ 个等概率符号中均匀选择。如果 $\text{PPL} = 20$，模型的不确定性下降到原来的五分之一——平均意外度为 $\log_2 20 \approx 4.32$ 比特，等同于约20个等概率选择。当 $\text{PPL} = 1$ 时，模型完全确定（每一步的意外度为0比特）；当 $\text{PPL} = V$（词表大小）时，模型等同于均匀随机猜测。
...
对于自回归语言模型，困惑度还可以按照生成方向报告：从左到右困惑度（每个词仅看到左侧上下文）和双向困惑度（每个词看到左右两侧上下文）。双向困惑度总是低于从左到右困惑度，差距的大小反映了模型从左到右的因果约束的严格程度。

**困惑度的历史与经验数据。** 困惑度作为评估指标的历史与语言模型本身同步。早期n-gram模型的困惑度通常在数百量级（如trigram on WSJ约为100到200）。神经语言模型的出现显著降低了困惑度：Bengio的NNLM将 困惑度 降至约200以下；Mikolov的RNNLM将 困惑度 降至约100以下。进入预训练时代后，大规模Transformer模型的困惑度进一步骤降：GPT-2在**Penn Treebank（PTB）等标准基准**上报告的困惑度约为**35.8**（常被引用的约35–36数字多指此类评测集，而非WebText训练困惑度）；WebText上的训练/内在困惑度若另有报告（如约37），应明确数据来源与定义，避免与PTB测试困惑度混写。GPT-3在多种基准上的困惑度约为20到30。然而，正如前面所述，这些数字的下界由语言的熵率决定——目前最优模型的困惑度仍远高于理论上限，这意味着自然语言的建模仍有巨大的改进空间。
...
其中类转移 $P(c_t \mid c_{t-1})$ 是**类 bigram**（一阶马尔可夫），发射 $P(w_t \mid c_t)$ 为类内词分布；二者均由计数估计，$P(w_t \mid c_t) = c(c_t, w_t)/c(c_t)$。这是Brown模型的关键结构：它保留类序列的动态依赖，而不是把类当作 i.i.d. 隐变量。最大化上述似然等价于在类划分下最大化相邻类的互信息（再减去类内词熵等项），这也是贪婪合并算法的信息论动机。

在固定类数的扁平模型中，可用 EM 估计参数：E 步在类 bigram HMM 上做前向–后向，得到后验 $P(c_t, c_{t-1} \mid w_1^T)$；M 步更新 $P(c'|c)$ 与 $P(w \mid c)$。实际的 Brown 算法则以硬聚类 + 贪婪合并实现同一目标，输出二叉树状的层次类结构。
...
步骤1（初始化）：词汇表中的每个词自成一类，形成 $V$ 个初始类。单词语类的先验/频率应取**词的经验频率** $P(c)=c(w)/N$（语料总词数 $N$），而非均匀 $1/V$。

步骤2（类对评估）：Brown clustering（Brown et al., 1992）以**类 bigram 平均互信息（AMI）**为目标：贪婪合并使合并后相邻类互信息最大化（等价于在类划分下最大化类 bigram 似然）。合并 $c_i,c_j$ 为 $c_{ij}$ 后，用类转移与发射计数重估 $P(c'|c)$、$P(w|c)$，并计算合并前后 AMI 之差 $\Delta\mathrm{AMI}$ 作为增益——**不是**均匀类概率，也不是 $\sum_w c(w)\log[P(w|c_{ij})/\max(P(w|c_i),P(w|c_j))]$ 这类 max 似然增益式。

步骤3（贪婪选择）：在所有候选类对中，选择 $\Delta\mathrm{AMI}$ 最大的一对合并（实现上常对共现类对剪枝并用堆加速）。每步候选评估最坏 $O(V^2)$，随合并进行减少。
...
**计算复杂度与可扩展性。** 贪婪合并的核心瓶颈在于步骤2的 $O(V^2)$ 类对评估。对于 $V = 100,000$，初始步骤需要评估约 $5 \times 10^9$ 个类对，这在计算上不可行。实践中采用多种近似策略：首先，只评估那些至少在语料中共现过的类对（使用共现矩阵的稀疏性）；其次，使用堆结构维护候选合并的优先级；第三，对高斯基模等快速近似方法进行预筛选。尽管如此，Brown聚类的完整实现仍然需要大量计算资源，这也是它被后来的神经方法部分取代的原因之一。
...
其中 $\sigma(x) = 1 / (1 + e^{-x})$ 是sigmoid函数，$\mathbf{v}_w$ 和 $\mathbf{v}_c$ 分别是从输入和输出嵌入矩阵中查得的向量。NEG 可写为**最大化**目标（正样本高分、负样本低分）：

$$\mathcal{J}_{\text{NEG}}^{\max}(w, c) = \log \sigma(\mathbf{v}_w^{\top} \mathbf{v}_c) + \sum_{k=1}^K \mathbb{E}_{w_k \sim P_n} \left[ \log \sigma(-\mathbf{v}_w^{\top} \mathbf{v}_{w_k}) \right]$$

等价地，**最小化**负对数形式（二分类交叉熵）为 $\mathcal{J}_{\text{NEG}}^{\min}=-\mathcal{J}_{\text{NEG}}^{\max}$：

$$\mathcal{J}_{\text{NEG}}^{\min} = -\mathbb{E}_{(w,c) \sim P} \left[ \log \sigma(\mathbf{v}_w^{\top} \mathbf{v}_c) \right] - \mathbb{E}_{(w, w_k) \sim P \times P_n} \left[ \log \sigma(-\mathbf{v}_w^{\top} \mathbf{v}_{w_k}) \right]$$

全文应只对一种形式做梯度上升或下降，勿把同一符号 $\mathcal{J}_{\text{NEG}}$ 既当最大化又当最小化。
...
在层次Softmax中，输出词表 $\mathcal{V}$ 的每个词 $w$ 对应于从根节点到叶节点的一条唯一路径，路径上的内部节点序列记为 $n(w, 1), n(w, 2), \ldots, n(w, l(w)-1)$（根到叶，不含叶本身），其中 $l(w)$ 是路径长度。每个内部节点 $n$ 关联一个参数向量 $\mathbf{u}_n \in \mathbb{R}^N$。定义路径上的二分类标签：对每个内部节点 $n(w, j)$，令 $d(w, j) \in \{+1, -1\}$ 表示从该节点走向 $w$ 时选择的分支（例如左子为 $+1$、右子为 $-1$）。**不可**使用 $\{0,1\}$ 指示：若标签取 $0$，则 $\sigma(0\cdot \mathbf{u}^\top\mathbf{h})=\sigma(0)=1/2$，无法表达另一分支。

给定隐藏状态 $\mathbf{h}$（Skip-gram中为中心词嵌入 $\mathbf{v}_w$），层次Softmax的输出概率为：

$$P(w \mid \mathbf{h}) = \prod_{j=1}^{l(w)-1} \sigma\left( d(w, j) \cdot \mathbf{u}_{n(w, j)}^{\top} \mathbf{h} \right)$$

该公式的含义是：沿着从根到 $w$ 的路径，在每一步用 sigmoid 计算“按 $d(w,j)=\pm 1$ 所指分支前进”的概率，再将路径上各步概率相乘。因为 $\sigma(-x)=1-\sigma(x)$，对侧分支概率自动互补；是否“走对分支”取决于当前参数 $\mathbf{u}_n$ 与 $\mathbf{h}$ 的内积符号与大小。
...
## 第6节 早期神经机器翻译：Encoder-Decoder与注意力机制

### 第6.1节 Encoder-Decoder架构的雏形
...
通常使用 $N = 4$（即计算1-gram到4-gram的精度）和均匀权重 $w_n = 1/N$。$N=4$ 的选择反映了以下直觉：1-gram精度衡量"信息覆盖度"（候选词是否出现在参考中），4-gram精度衡量"词序准确性"（四个词的序列是否一致），而2-gram和3-gram精度衡量"局部流畅性"。使用几何平均而非算术平均的原因在于：几何平均对任一 $p_n=0$ **严格归零**（$\prod p_n^{w_n}=0$），从而强制候选在所有 n-gram 级别均有非零重叠；若改用算术平均，即使 $p_4=0$，只要 $p_1,p_2,p_3>0$ 仍可得到正分，无法同样严厉地惩罚高阶 n-gram 完全失配。因此 BLEU 选用几何平均（再乘 BP）作为综合精度。
...
4. **对长度的不敏感性（过长翻译）：** 按定义，仅当 $c \leq r$ 时 $\text{BP}=\exp(1-r/c)$；当 $c > r$ 时 $\text{BP}=1$，过长候选不受简洁性惩罚。因此过长翻译（如重复参考内容）不会因长度被压分，只可能通过修改精度间接下降——它们仍可能获得与长度适中的翻译相近甚至更高的BLEU分数。
...
$$\text{ROUGE-N} = \frac{\sum_{\text{n-gram} \in \mathcal{R}} \min\!\big(\mathrm{count}(\text{n-gram}, \mathcal{R}),\, \mathrm{count}(\text{n-gram}, \hat{\mathbf{y}})\big)}{\sum_{\text{n-gram} \in \mathcal{R}} \mathrm{count}(\text{n-gram}, \mathcal{R})}$$
