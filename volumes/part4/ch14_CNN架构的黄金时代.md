...
2012年AlexNet在ImageNet上的突破性表现标志着深度学习进入了崭新纪元，但真正将卷积神经网络从概念验证推向工程艺术的，是2014至2016年间发生的一场静默而深刻的架构革命。要理解这一革命的紧迫性，需要回顾2012-2014年间的技术背景。AlexNet（Krizhevsky等人，2012）在ILSVRC 2012上将Top-5错误率从约26.2%（传统方法亚军量级）降至约16.4%（5-CNN平均），证明了GPU训练深度卷积网络的可行性。然而AlexNet的架构设计相当特化——两个GPU之间通过交叉连接通信，不同层使用不同大小的卷积核，全连接层占据了绝大部分参数。研究者面临的核心挑战是：AlexNet的突破究竟来自于"深度"本身，还是来自于其特定的架构选择？如果更深能带来更好的性能，那么"更深"的上限在哪里？有没有系统性的方法可以设计更深的网络？
...
VGG-19的全部参数分布如下：卷积层（16个conv）参数量约20M（在VGG-16的13个卷积层基础上，conv3_3之后增加1层约0.59M参数（256通道）、conv4_3与conv5_3之后各增加1层约2.36M参数（512通道）），三个全连接层（fc6, fc7, fc8）共包含约123.6M参数，总计约143.7M可训练参数（卷积约20M + 全连接约123.6M）。
...
**Inception系列的完整演化：从v1到Inception-ResNet。** Inception v1（GoogLeNet, 22层）证明了多尺度并行路径的有效性，但其设计中存在一些不一致之处——例如，Inception模块中3×3池化和5×5卷积使用了不同的padding策略，导致不同路径的输出尺寸需要仔细对齐。Inception v2（2015）解决了这些问题，并将5×5卷积替换为两个连续的3×3卷积，同时引入了批归一化——这是BN在大型视觉模型中的首次成功应用。Inception v2在ImageNet上取得了约4.9%的Top-5错误率（使用多裁剪，**单模型**量级），相对 GoogLeNet **单模型单裁剪约10.07%**有明显提升；不宜与 GoogLeNet 的 **7 模型×多裁剪集成 6.67%** 直接写成“优于”，二者评估协议不同。

Inception v3（2015）进一步将n×n卷积分解为1×n和n×1的非对称卷积，并将网络深度增加到48层（含参数层）。Inception v3引入了**标签平滑（label smoothing）**——将 hard one-hot 替换为 $y'_k=(1-\varepsilon)\delta_{k,y}+\varepsilon/K$（$K=1000$、$\varepsilon=0.1$ 时真类约 $0.9001$、其余约 $0.0001$），不宜写成 $[0.1/999,\,0.9,\,0.1/999,\ldots]$（分母用 $K-1$ 且漏掉真类上的 $\varepsilon/K$）。Inception v3在ImageNet上单裁剪Top-5错误率约为5.6%，单模型多裁剪Top-5约为3.58%。
...
这一等式的直观意义在于：第一项中的单位矩阵$\mathbf{I}$是恒等快捷连接带来的"梯度旁路"——反向传播中出现 $\mathbf{I}+\partial\mathcal{F}/\partial\mathbf{x}_l$ 结构，使梯度不必完全依赖残差支路的链式乘积。**但这并不构成梯度范数的下界保证。** 矩阵 $\mathbf{I}+J_F$ 的最小奇异值完全可能为 0（例如 $J_F=-\mathbf{I}$），谱半径与奇异值也不可混用；恒等路径改善了优化景观与经验可训练性，却不能写成“梯度始终以幅度 1 传播”或“有保证的最小梯度流”。

换句话说，恒等快捷连接使得梯度传播被分解为两条路径：
1. **短路径（shortcut）：** 梯度直接通过$\mathbf{I}$前传，不经过任何非线性变换层
2. **长路径（residual）：** 梯度通过残差函数$\mathcal{F}$前传，经历正常的链式法则

即使长路径的梯度衰减严重，短路径仍提供额外通路；这种冗余在经验上显著改善了极深网络的可训练性，也是 ResNet 能训练数百层网络的重要原因之一。可用标量玩具例子建立直觉：若无残差、每层缩放因子为 $\lambda=0.9$，则 20 层衰减约 $0.9^{20}\approx0.122$，100 层约 $0.9^{100}\approx2.7\times10^{-5}$。有残差时，单层反向映射形如 $1+\lambda_F$，在 $|\lambda_F|$ 不太接近 $-1$ 时往往更不易被连乘压扁。但矩阵情形下各层 $J_F$ 可交换与否、奇异值分布、以及与 BatchNorm/初始化的交互都很复杂；**不能**从“存在 $\mathbf{I}$”推出每层梯度范数 $\ge1$，也不能把 ResNet 的成功单一归因于“有保证的最小梯度流”。更稳妥的表述是：恒等捷径提供了不经过深层非线性复合的直达路径，改善了梯度传播与损失景观，具体定量保证需要额外假设（如对 $J_F$ 的谱/奇异值约束）。
...
预激活设计的主要效果是：**恒等快捷路径**不经过块末端的ReLU非线性，从而提供更干净的梯度通路（残差支路本身仍包含BN、ReLU与卷积运算）；这**改善了**整体的梯度传播条件，是训练极深网络（如1001层）的关键设计之一，但不宜表述为"保证无损"（完整梯度仍包含残差支路、投影捷径和维度变化等因素）。He等人使用预激活ResNet成功训练了1001层的网络，在CIFAR-10上测试错误率约**4.62%**（Identity Mappings in Deep Residual Networks, He et al., 2016），说明极深残差网络在小数据集上仍可达到很强精度，而非退化到失败。

Wide Residual Networks（WRN, Zagoruyko和Komodakis，2016）则从另一个角度拓展了残差网络：不增加深度，而是增加每层的通道数。实验表明，一个宽度为原来的k倍、深度仅为原始ResNet二分之一的网络，可以取得更优的性能——例如WRN-28-10（28层，宽度因子10）的CIFAR-10错误率约**4.00%**，优于预激活 ResNet-1001 的约 **4.62%**（He et al., Identity Mappings, 2016；后者深度约为其 36 倍。若引用 WRN 文中其他 ResNet-1001 设定/复现数字，须单独标明出处，不可与 4.62% 混为同一结果）。这一发现挑战了"深度是最关键因素"的假设，表明宽度和深度之间存在可以互补的权衡。
...
Highway Networks与ResNet的核心区别在于门的引入。ResNet的恒等快捷连接是无门控的——梯度通过恒等路径获得从深层到浅层的直达通路，这改善了优化条件，**但不构成"始终以完整幅度传播"的保证**（参见第4.4节关于I+J_F最小奇异值可能为0的讨论）。恒等映射使得ResNet的数学分析更加清晰（梯度流中存在确定的恒等项$\mathbf{I}$），训练也更加稳定。然而，门控机制的灵活性在高层次语义推理任务中可能具有优势——Srivastava等人报告Highway Networks在语言建模任务上优于同等深度的残差网络。对于视觉任务，ResNet的无门控设计在ImageNet上取得了更好的结果，但门控思想在后续的LSTM和GRU（以及Transformer中的门控注意力机制）中得到了广泛的应用。
...
ResNet在ILSVRC 2015上取得了Top-5错误率3.57%的成绩（**6个不同深度残差网络的集成**，其中仅包含两个152层模型）——在ImageNet测试集上进一步显著超越人类水平（人类Top-5错误率约为5%；“首次”超过人类的里程碑更宜归于 He et al. 2015 约 4.94% 的 PReLU 结果）。单模型ResNet-152在验证集上的多尺度评估结果为Top-1 19.38%、Top-5 4.49%（He等人，2016，Table 4），10裁剪评估则为Top-1 21.43%、Top-5 5.71%（Table 3）。这一里程碑事件证明了深度残差学习的强大威力，也标志着CNN架构研究进入了一个全新的阶段。
...
如果没有残差连接（即标准卷积网络），反向传播的雅可比就是$\partial \mathcal{F}/\partial \mathbf{x}_l$本身，其奇异值若长期小于 1，链式乘积会导致梯度指数衰减。有了恒等项$\mathbf{I}$，映射变为 $\mathbf{I}+J_F$，在**标量或可对角化且与稳定性分析对齐**的简化模型中，可把特征值整体平移 1 来建立直觉；但一般矩阵情形下：特征值实部、谱半径与控制梯度放缩的奇异值不是同一对象；ReLU 后雅可比也**没有**“特征值通常为负”的一般定理。残差连接把系统推向更易优化的区域是经验与部分理论工作的结论，应避免写成对任意深度网络都成立的谱保证。
...
ShuffleNet v2（2018）进一步指出，分组卷积中的通道混洗操作和逐点分组卷积的通道数瓶颈（1×1卷积占据大部分计算量）限制了效率。ShuffleNet v2通过通道分裂（channel split）策略——在残差块的每个分支中，将输入通道分为两部分，一部分保持恒等映射，另一部分进行卷积——在保持精度的同时进一步降低了计算量。在ImageNet上，ShuffleNet v2 约 **72.6% Top-1** 对应 **1.5× 配置约 299 MFLOPs（≈0.3G）**，而非 0.5G；约 **0.5–0.59G** 量级的是 **2×**（约 **74.9% Top-1**）。
...
*注：除非另有说明，错误率为单模型结果。AlexNet为单模型 **10-view（5 crop + flip）**（Krizhevsky等人，2012；非中心裁剪）；VGG为单尺度$S=Q=256$（Simonyan和Zisserman，2015，Table 3，配置D/E）；ResNet为10裁剪验证集结果（He等人，2016，Table 2–3），其FLOPs取自原文Table 1（乘加次数）；GoogLeNet的†6.67%为7模型×144裁剪集成测试结果，单模型单裁剪约10.07%（Szegedy等人，2015，Table 3）；Inception-v3为单帧评估（Szegedy等人，2016）；MobileNet为Top-1准确率换算的错误率，FLOPs按原文Mult-Adds计（Howard等人，2017；Sandler等人，2018）。不同论文对FLOPs/Mult-Adds计数约定不一，横向比较时需注意。*
...
- **单裁剪（single-crop）：** 从图像中裁剪一个224×224区域进行评估，速度快，但精度通常**低于**多裁剪/密集评估（更保守，而非更乐观）。
