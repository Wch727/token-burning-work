...
| Round 6 | **6 组全量复审** 24 章（非抽检、无条数上限） | ~79 条 high（AlexNet 口径/DBN 拓扑/公式/法规编号/交叉引用等） | 已修（本轮） |
| 脚本 | review-r8 + review-r10 | 0 findings | 通过 |

**停止准则**：R5 起恢复全量通读；R6 全量复审后脚本仍 0 findings。可再开 R7 确认是否返回 `[]` 或仅剩非操作性意见。
...
**近期相关 commit（本机）**：
```
（本轮）fix: R6 full re-read — high-confidence hard errors across 24 chapters
