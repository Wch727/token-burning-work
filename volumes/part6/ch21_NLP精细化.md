...
**实验结果。** Toolformer 以 **GPT-J（6.7B）** 为基础模型（而非 GPT-2 large 的 774M）。Schick et al.（2023）在零样本设置下报告：接入工具后，模型在数学类基准（如 ASDiv、SVAMP、MAWPS）以及 WebQuestions 等需要外部知识/计算的任务上相对无工具基线有显著提升；原文主结果并非以 GPT-2-large 在 GSM8K 上“8.6%→42.5%”这类数字给出，不宜沿用错误基座与虚高百分比。Toolformer 的核心贡献在于展示：通过自监督筛选工具调用数据再微调，模型可自主学会何时、如何调用工具，而无需人工标注工具调用轨迹。
