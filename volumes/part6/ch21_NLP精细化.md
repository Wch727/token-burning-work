...
**softmax注意力的分解。** 为了处理softmax的归一化分母，Performer将softmax核写成高斯核与径向因子的乘积分解：
...
总HBM访问量为$O(n^2)$量级（中间注意力矩阵的读写占主导）。FlashAttention的目标是通过分块（tiling）技术，将HBM访问量降至$\Theta(N^2 d^2 / M)$，其中$M$为SRAM容量——当$M$足够大时，显著低于朴素实现的$O(N^2)$次中间矩阵读写。
...
**IO复杂度分析。** 设SRAM容量为$M$。FlashAttention 取块大小 $B_r \approx \Theta(M/d)$（使 $Q_i$ 等块可放入 SRAM），而非 $\min(d,\sqrt{S})$。每个块的 $S_{ij}=Q_i K_j^\top$ 在 SRAM 中计算，不将完整 $n\times n$ 矩阵写回 HBM。论文给出的 HBM 复杂度为 $\Theta(N^2 d^2/M)$：当 $M$ 增大时 HBM 访问减少，与朴素 $O(N^2)$ 中间矩阵读写形成对比。
...
|------|----------|----------|------------|------------|
| Linformer | 低秩投影 | 低秩近似 | O(nk) | 2-5× |
| Performer | 随机核特征 | 核近似 | O(nm) | 2-4× |
| FlashAttention | IO优化 | 精确计算 | 计算 $\Theta(N^2 d)$；HBM IO $\Theta(N^2 d^2/M)$ | 4-8× |
...
其中$\bar{A}, \bar{B}$为依赖于输入x的时间-varying参数（这是"选择性"的关键）。在离散化形式下，使用零阶保持（ZOH）方法，设步长为$\Delta$，连续系统参数为$A,B$，则：

$$
\bar{A} = \exp(\Delta A),\quad \bar{B} = A^{-1}(\exp(\Delta A)-I)B,\quad h_t = \bar{A} h_{t-1} + \bar{B} x_t,\quad y_t = C^\top h_t + D x_t
$$

（当$A$可对角化时，$\bar{B}$也可写为等价的积分形式；选择性SSM中$\Delta,A,B,C$均可随输入变化。）当$\bar{A}$的特征值模小于1时，历史信息呈指数衰减，模型天然具备选择性遗忘机制，无需像注意力机制那样显式地存储所有历史token。这使Mamba在长序列建模上具有O(n)的理论复杂度，同时在语言建模任务上取得了与同规模Transformer相当的性能。
...
**实验验证。** Yao等人（2023）在Game of 24、创意写作和跨词逻辑推理任务上验证了ToT的有效性。在24点游戏中，标准输入-输出（IO）提示约7%，CoT约4%，ToT在束宽$b=1$时约45%，$b=5$时约74%。注意：约7%对应的是IO基线，而非ToT在$b=1$时的结果。这证明了系统化树搜索相比单路径采样在复杂推理任务上的显著优势。
...
**阶段一：领域自适应预训练（Domain-Adaptive Pretraining, DAPT）。** 从**对应规模**的 GPT 类权重继续训练（公开报告最大约 12B），**并非**从原始 GPT-3 175B checkpoint 直接续训；在此基础上使用过滤后的代码数据继续预训练$L_2$个token。训练目标为标准语言建模目标：
...
Codex在HumanEval上达到28.8%的pass@1和约72.3%的pass@100（Codex-12B）。相比之下，Codex 论文主表中未做代码微调的 **GPT-J 6B** 约 **11.6%** pass@1（不宜写成 GPT-3 = 11.4%）。值得注意的是，Codex在涉及库使用的任务（如使用numpy、pytorch）上表现显著优于纯算法任务，这是因为训练数据中包含了大量使用这些库的实际代码。
...
- 光标前后上下文窗口（实践中常取约数千字符/token量级的窗口，而非“$N_c$行=$6000$字符”这类量纲混写）
...
$$
x_{\text{FIM}} = \langle\text{fim\_prefix}\rangle\,[\text{pre}]\,\langle\text{fim\_suffix}\rangle\,[\text{suf}]\,\langle\text{fim\_middle}\rangle\,[\text{middle}]
$$

模型在 $\langle\text{fim\_middle}\rangle$ 之后生成中间片段。训练目标为：

$$
\mathcal{L}_{\text{FIM}} = -\sum_{t=\text{start}}^{\text{end}} \log p_\theta(x_t | x_{\text{FIM}, <t})
$$

在推理时，将用户代码拆分为前缀（pre）和后缀（suf），按 prefix–suffix–middle 顺序拼接特殊标记，模型在 $\langle\text{fim\_middle}\rangle$ 后填充中间代码。FIM使Copilot在代码修改和重构场景下的准确率提升了约15-20个百分点。
...
训练使用900GB数据（包括The Stack的86%和约400GB的精选自然语言数据），在 **512** 张 A100 GPU（80GB）上训练约 24 天。

**StarCoder：Python 领域续训变体。** 在 StarCoderBase 基础上，Li et al. (2023) 用约 **35B Python token** 继续预训练得到 StarCoder（领域续训，而非约 2.2 万条自指令微调）。后续 WizardCoder 等才以指令数据微调路线为主；若写指令微调须改指那些工作并更新引用。
...
**多模态代码理解。** 代码不仅仅是文本——它包含结构信息（AST）、控制流信息（CFG）、数据流信息（DFG）以及UI渲染信息（前端代码的屏幕截图）。将代码的多种模态信息统一到预训练框架中，是未来的重要方向。TransCoder-ST（Rozière et al., 2021）等多语言代码翻译模型展示了结构信息在跨语言代码迁移中的价值。
...
1. 使用更大的模型（XLM-R large 约550M参数；不宜与 RoBERTa-large 的约355M混为一谈）
...
**模型规模扩展与低资源表现。** mT5-xxl的约13B参数使得模型在低资源语言上的表现显著提升。在XGLUE（Liang et al., 2020）基准上，mT5-xxl在9种语言的文本分类任务上平均达到80.5 F1，比XLM-R-large提升约5 F1点。mT5还展示了在零样本跨语言摘要和问答任务上的能力——这些生成任务在BERT架构下难以直接处理。
...
**多语言训练数据。** PaLM 约在 780B tokens 的混合语料上训练。按论文报告的大致配比：社交媒体约 **50%**、过滤网页、书籍、维基百科、新闻，以及 **GitHub 代码约 5%** 等（不宜写成社交媒体 55% 且漏掉代码语料）。英语约占总体的约 78%，其余为代码与多语言自然文本。
...
**SPLADE：稀疏语义检索。** SPLADE（SParse Lexical AnD Expansion model，Formal et al., 2021）结合了稀疏检索的可解释性和密集检索的语义理解能力。SPLADE 使用 BERT 的 **MLM 头**在各 token 位置预测词表 logits，再经 ReLU 与 log-saturation 等变换后沿序列维聚合为稀疏词项权重向量——**并非**在 [CLS] 上接 $|V|$ 维分类头，也**不以** Gumbel-Softmax 为主机制。

给定查询q，SPLADE输出一个稀疏向量$v_q \in \mathbb{R}^{|\mathcal{V}|}$，其中大部分元素为零。文档d同样编码为$v_d \in \mathbb{R}^{|\mathcal{V}|}$。相似度计算为：

$$
\text{sim}_{\text{SPLADE}}(q, d) = \sum_{t \in \mathcal{V}} v_q[t]\, v_d[t]
$$

即两个稀疏向量的点积（激活后权重非负）。这等价于扩展后的查询词项与文档词项在共享维度上的加权重叠。
...
**RAG与生成式检索的对比。** 近年来的生成式检索（Generative Retrieval）提出直接使用语言模型生成文档标识符（如文档标题或特殊token序列），而非检索文档向量。代表性工作包括DSI（Tay et al., 2022）、NCI（Wang et al., 2022）以及面向实体检索的GENRE（De Cao et al., 2021）等。研究表明，生成式检索在封闭域任务上具有竞争力，但在开放域大规模检索中，检索-排序管线（retrieve-then-rerank）仍是最可靠的方法。
...
```
Question: 一个存储了2L、浓度为3mol/L的溶液，需要浓缩到1L，求最终浓度是多少？

Thought 1: 我需要使用溶质守恒公式 C1 × V1 = C2 × V2。
...
Answer: 使用溶质守恒公式，C2 = C1 × V1 / V2 = 3 × 2 / 1 = 6 mol/L。
```
...
**工具API设计。** Toolformer 集成了**五类**工具（Schick et al., 2023）：
1. **计算器（Calculator）：** 执行基本算术运算
2. **问答系统（Question Answering）：** 回答事实性问题
3. **搜索引擎（Search Engine / Wikipedia Search）：** 返回检索结果摘要
4. **翻译系统（Translation System）：** 执行机器翻译
5. **日历（Calendar）：** 处理日期/时间相关查询
...
**实验结果。** Toolformer 以 **GPT-J（约 6B，≈6.05B）** 为基础模型（与 §3.1 中 GPT-J 6B 表述一致；不宜写成 6.7B，亦非 GPT-2 large 的 774M）。Schick et al.（2023）在零样本设置下报告：接入工具后，模型在数学类基准（如 ASDiv、SVAMP、MAWPS）以及 WebQuestions 等需要外部知识/计算的任务上相对无工具基线有显著提升；原文主结果并非以 GPT-2-large 在 GSM8K 上“8.6%→42.5%”这类数字给出，不宜沿用错误基座与虚高百分比。Toolformer 的核心贡献在于展示：通过自监督筛选工具调用数据再微调，模型可自主学会何时、如何调用工具，而无需人工标注工具调用轨迹。
...
**SuperGLUE的升级。** GLUE发布后不久，顶级模型的性能迅速饱和——到2019年，人类基线（87.1分）已被 **T5-11B 约 90.3 分** 等模型超越（**89.3 是 T5 在 SuperGLUE 上的分数**，勿与 GLUE 混用）。这表明GLUE的区分度不足。SuperGLUE（Wang et al., 2019）通过以下方式提升了难度：
1. 移除已饱和的SST-2和QQP等任务，以更具挑战性的任务集替换，主要包括：
   - **BoolQ：** 基于段落的是/否问答
   - **CB（CommitmentBank）：** 三分类自然语言推理
   - **COPA（Choice of Plausible Alternatives）：** 因果推理，需要理解事件之间的因果关系
   - **MultiRC（Multi-Sentence Reading Comprehension）：** 多句阅读理解，需要跨句整合信息
   - **ReCoRD：** 需要常识推理的填空式阅读理解
   - **RTE：** 识别文本蕴含
   - **WiC（Words in Context）：** 上下文词义消歧
   - **WSC（Winograd Schema Challenge）：** 指代消解
2. 强调更难的推理与常识整合，而非简单扩大既有GLUE任务规模（SuperGLUE任务集不含MNLI）
3. 减少数据泄漏与过易捷径，提升排行榜区分度

SuperGLUE的人类基线为89.8分，而论文给出的BERT基线约**69.0**、BERT++约**71.5**（并非约76分），相对人类仍有约18–20分的提升空间。
...
人类专家的平均准确率约为89.8%（通过众包平台汇集多位专家的标注），而GPT-3（175B）仅为43.9%，Chinchilla（70B）约 **67.5%**（5-shot，Hoffmann et al.）。这表明MMLU对模型的专业深度提出了很高要求。
...
**检索增强作为事实性保障。** 最直接的缓解幻觉方法是将生成锚定在检索文档上。RAG框架（见第5节）通过提供检索到的相关文本作为生成上下文，使模型有据可查。然而，即使提供了检索文档，模型仍可能生成与文档不一致的内容。提高RAG忠实性（faithfulness）的方法包括：
...
这种方法的一个关键假设是：amateur模型不知道expert模型的"幻觉知识"，因此幻觉token在两个模型上的概率差异较小（比值接近1），而非幻觉的知识token在expert上概率远高于amateur（比值显著大于1）。实验表明，在long-form生成任务上，对比解码将FActScore从约28提升至35（PaLM 540B），同时保持了生成文本的流畅度。
...
- **RARR（Gao et al., 2023）：** Researching and Revising What Language Models Say（长文 QA 归因与修订框架），评测生成文本的事实准确性、attributedness 与检索利用率
