...
预激活结构使得梯度在反向传播中经过残差块时需经过加法节点和权重矩阵，但不经过ReLU非线性——这**改善了**梯度的传播条件，是训练极深网络（如1001层）的关键设计之一，不宜表述为"保证无损"（完整梯度仍包含残差支路、投影捷径和维度变化等因素）。He等人使用预激活ResNet成功训练了1001层的网络，在CIFAR-10上测试错误率约**4.62%**（Identity Mappings in Deep Residual Networks, He et al., 2016），说明极深残差网络在小数据集上仍可达到很强精度，而非退化到失败。
