...
VGG-19的全部参数分布如下：卷积层（13个conv）参数量约14.7M，其中前两个3×3卷积层（conv1_1, conv1_2，输入通道3，输出通道64）每层约1,792个参数（2 × 3 × 3 × 3 × 64 + 64），两层共计约3,584个参数（含偏置）。三个全连接层（fc6, fc7, fc8）共包含约123.7M参数（fc6: $7 \times 7 \times 512 \times 4096 + 4096 \approx 102.8\text{M}$，fc7约16.8M，fc8约4.1M），总计约138.4M可训练参数。
