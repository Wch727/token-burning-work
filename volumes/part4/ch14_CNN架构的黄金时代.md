...
2012年AlexNet在ImageNet上的突破性表现标志着深度学习进入了崭新纪元，但真正将卷积神经网络从概念验证推向工程艺术的，是2014至2016年间发生的一场静默而深刻的架构革命。要理解这一革命的紧迫性，需要回顾2012-2014年间的技术背景。AlexNet（Krizhevsky等人，2012）在ILSVRC 2012上将Top-5错误率从26.1%（传统方法）降至16.4%，证明了GPU训练深度卷积网络的可行性。然而AlexNet的架构设计相当特化——两个GPU之间通过交叉连接（cross-GPU connections）通信，不同层使用不同大小的卷积核，全连接层占据了绝大部分参数。研究者面临的核心挑战是：AlexNet的突破究竟来自于"深度"本身，还是来自于其特定的架构选择？如果更深能带来更好的性能，那么"更深"的上限在哪里？有没有系统性的方法可以设计更深的网络？
...
**ImageNet竞赛的演进与黄金时代的定位。** ILSVRC从2010年举办至2017年，每年吸引全球顶尖的研究团队参与竞争。2010-2011年的冠军使用传统的计算机视觉方法（SIFT特征 + Fisher向量 + SVM分类），Top-5错误率在28%以上。2012年AlexNet的16.4%标志着深度学习时代的到来。2014年VGGNet（7.3%）和GoogLeNet（6.67%）将错误率降至个位数。2015年ResNet（3.57%）首次超越人类水平。2016年ensemble方法将错误率降至约3%。2017年SENet（2.25%）标志着ImageNet竞赛的"人类水平"极限已被彻底突破。从2018年开始，ILSVRC不再举办竞赛——因为ImageNet分类任务的"基准"问题已被认为基本解决，研究焦点转向了更复杂的视觉任务（检测、分割、视频理解）和新的研究范式（自监督学习、少样本学习）。黄金时代的架构创新——VGG、GoogLeNet、ResNet——正是这一演进过程中最关键的几个里程碑。
...
AlexNet的成功证明了深度卷积神经网络的巨大潜力，但其架构设计相对碎片化——不同层使用了不同大小的卷积核（11×11、5×5、3×3），全连接层与卷积层交替出现，整体结构缺乏统一的设计范式。Simonyan和Zisserman（2014）在其 seminal 工作中提出了一个根本性问题：**如果我们放弃一切非必要设计，仅用最基础的构件——3×3卷积和最大池化——反复堆叠，能走多远？**
...
VGG-19的完整参数分布如下：卷积层总参数量约144M，其中前两个3×3卷积层（conv1_1, conv1_2，输入通道3，输出通道64）贡献了约1,792个参数（2 × 3 × 3 × 3 × 64），三个全连接层（fc6, fc7, fc8）各包含约102M参数（7×7×512×4096 ≈ 102.76M），总计约144M可训练参数。
...
**VGG的迁移学习遗产。** VGGNet的一个被低估的贡献是其作为迁移学习主干网络（backbone）的卓越表现。由于VGG的架构高度规整——所有卷积层使用相同的3×3核和相同的padding策略——其特征图具有高度的空间对齐性和语义一致性。当使用VGG-16/19作为预训练主干网络，在PASCAL VOC、COCO等小规模数据集上进行微调（fine-tuning）时，VGG consistently 优于同期的GoogLeNet和ResNet-50（在微调场景下）。这一现象的解释在于：规整的架构更容易进行空间定位和像素级预测（如语义分割、目标检测），而Inception模块中不同路径产生的多尺度特征在对齐和映射到像素级输出时更加复杂。直到2017-2018年，随着FPN（特征金字塔网络）和Mask R-CNN等架构的出现，ResNet才在检测和分割任务上全面超越了VGG。至今，VGG-16仍是许多迁移学习教程和研究论文的标准baseline主干网络。

**VGG与感受野的极限。** 如前所述，VGG-16最终的特征图尺寸为7×7（经过5次2×2池化，224→112→56→28→14→7），感受野覆盖了260×260的区域——远超输入图像的224×224尺寸。这意味着VGG的全连接层看到的不是局部特征，而是"全局上下文"。然而，这种全局性是有代价的：7×7的空间分辨率在目标检测和语义分割等需要精细空间定位的任务中显得过于粗糙。7×7的特征图对应到原始224×224图像，每个特征单元覆盖32×32像素的区域——这对于检测小物体或 delineating 精确边界是不足够的。这一问题直接催生了后续的特征金字塔网络（FPN, Lin等人，2017）和空洞卷积（dilated/atrous convolution）等空间分辨率增强技术。
...
**感受野的精确计算。** 对于VGG-16，从输入图像到conv3_3输出的感受野计算如下。每个3×3卷积层的感受野为3，池化层为2。使用递推公式$\text{RF}_l = \text{RF}_{l-1} + (k_l - 1) \cdot \prod_{i=1}^{l-1} s_i$，其中$k_l$是第$l$层的核大小，$s_i$是第$i$层的步长：

- conv1_1: RF = 3
- conv1_2: RF = 3 + (3-1)×1 = 5
- pool1: RF = 5 + (2-1)×1 = 6
- conv2_1: RF = 6 + (3-1)×2 = 10
- conv2_2: RF = 10 + (3-1)×2 = 14
- pool2: RF = 14 + (2-1)×2 = 16
- ...逐层递推...
- conv4_3: RF = 46 + (3-1)×4 = 54
- pool4: RF = 54 + (2-1)×4 = 58
- conv5_1: RF = 58 + (3-1)×8 = 74
- conv5_2: RF = 74 + (3-1)×8 = 90
- conv5_3: RF = 90 + (3-1)×8 = 106
- pool5: RF = 106 + (2-1)×8 = 114

因此，VGG-16经过pool5后的总感受野为114×114。全连接层fc6基于这114×114的区域进行分类决策——这虽然已经覆盖了图像的一大部分，但**并未达到"全局"程度**（114 < 224）。fc6的参数总量（$114 \times 114 \times 512 \times 4096 \approx 27.3$亿）虽然仍然庞大，但比按260×260计算的$102 \times 102 \times 512 \times 4096$要小得多。这一修正反映了VGG架构的一个重要特点：即使堆叠多个3×3卷积层，全连接层仍然只能"看到"局部区域，这也是后来全卷积网络（FCN）放弃全连接层、改用全卷积设计的动机之一。

---

## 3. GoogLeNet：Inception模块与网络的宽度探索

### 3.1 从"一个卷积核大小够好吗？"到"为什么不全部用上？"
...
参数量从409,600骤降至53,248，压缩比约为7.7倍。同时，1×1卷积本身增加了网络的非线性能力（经过ReLU激活后），而池化路径保留了平移不变性的最强信号。所有路径的输出在通道维拼接后，网络可以自行学习不同尺度特征的权重分配。
...
**辅助分类器（Auxiliary Classifiers）：** 在Inception 3a和4a的输出之后，GoogLeNet各插入了一个辅助分类器。每个辅助分类器由一个5×5平均池化层（步长3）、一个1×1卷积（128通道）、两个全连接层（1024和1000节点，1000对应ImageNet类别数）以及一个Softmax层组成。辅助分类器的损失以0.3的权重加权加入总损失：
...
从计算量角度，GoogLeNet的FLOPs约为3.8G（单尺度前向传播），不到VGG-16（30.9G）的13%。如果使用多尺度评估（如VGG的多裁剪融合），GoogLeNet的总计算量仍远低于VGG。这一效率优势在移动端和嵌入式设备上尤为宝贵——虽然GoogLeNet并未专门为移动端设计，但其参数效率已经为后续MobileNet等移动端架构铺平了道路。

GoogLeNet在ILSVRC 2014上的Top-5错误率为6.67%（使用多尺度裁剪和融合），优于VGG-16的7.3%（多裁剪）。在Top-1错误率方面，GoogLeNet约为26.7%（多裁剪），VGG-16约为25.6%（多裁剪）——VGG-16在Top-1上略优，但GoogLeNet以更少的参数（6.6M vs. 138M）和更低的计算量（3.8G vs. 30.9G FLOPs）达到了接近的性能。GoogLeNet的优势主要体现在：（1）参数量仅为VGG-16的4.8%；（2）FLOPs仅为VGG-16的12%；（3）更深的网络结构（22层 vs. 16层）提供了更强的表征能力潜力。

### 3.4 Inception v2与v3：尺寸分解的思想
...
Inception v3（2015）进一步将n×n卷积分解为1×n和n×1的非对称卷积，并将网络深度增加到48层（含参数层）。Inception v3引入了**标签平滑（label smoothing）**——将hard one-hot标签替换为soft标签（例如，将[0, 1, 0, ...]替换为[0.1/999, 0.9, 0.1/999, ...]），减少了模型对错误标签的过度置信，提升了泛化能力。Inception v3在ImageNet上取得了4.2%的Top-5错误率（多裁剪）。

Inception-v4（2016）和Inception-ResNet-v2（2016）将Inception模块与残差连接深度融合。Inception-v4进一步加深了网络（70层），优化了Stem设计（使用三个3×3卷积替代7×7卷积），并引入了Reduction模块替代部分Inception模块以实现更高效的下采样。Inception-ResNet-v2则在每个Inception模块后添加残差连接——与ResNet的恒等快捷连接不同，Inception-ResNet的残差连接使用了1×1卷积（步长1）来匹配通道数，而非简单的恒等映射。这种设计在处理多路径拼接后的通道数变化时更加灵活。Inception-ResNet-v2在ImageNet上取得了3.0%的Top-5错误率（多裁剪），优于同期ResNet-152的3.57%，证明了Inception模块和残差连接的协同效应。
...
**Inception模块的通道维度详解。** 为了更具体地理解GoogLeNet的信息流，以下以Inception 3a模块为例，逐步追踪各路径的通道变化。输入为28×28×192的特征图（来自stem的输出）。路径1使用64个1×1卷积，输出28×28×64；路径2先通过32个1×1卷积降维至28×28×32，再通过128个3×3卷积输出28×28×128；路径3先通过32个1×1卷积降维至28×28×32，再通过32个5×5卷积输出28×28×32；路径4先经过3×3最大池化（步长1，padding为1以保持分辨率），再通过32个1×1卷积输出28×28×32。四条路径在通道维拼接后，输出为28×28×256（64+128+32+32=256）。整个Inception 3a模块的参数量仅为约15.9万——在提供256通道的多尺度特征的同时保持了极高的参数效率。
...
$$
\frac{\partial \mathcal{L}}{\partial \mathbf{x}_l} = \frac{\partial \mathcal{L}}{\partial \mathbf{x}_L} \cdot \left( \mathbf{I} + \frac{\partial \mathcal{F}(\mathbf{x}_l, \mathcal{W}_l)}{\partial \mathbf{x}_l} \right) + \frac{\partial \mathcal{L}}{\partial \mathbf{x}_{l+1}} \cdot \frac{\partial \mathcal{F}(\mathbf{x}_l, \mathcal{W}_l)}{\partial \mathbf{W}_l}
$$
...
即使长路径的梯度衰减严重，短路径始终存在。这种"冗余的梯度传播路径"是ResNet能够训练数百层网络的根本原因。为了更直观地理解这一机制，考虑一个简化的数值例子。假设一个残差块的残差函数雅可比矩阵的特征值为$\lambda = 0.9$（即梯度通过残差路径传播时衰减为原来的90%）。对于20层深的网络，不使用残差连接的标准网络，梯度从第20层传播到第1层的衰减因子为$0.9^{20} \approx 0.122$——梯度衰减到原来的约12%，虽未完全消失但已经相当微弱。对于100层网络，衰减因子为$0.9^{100} \approx 0.000026$——梯度几乎完全消失。而使用残差连接的网络，梯度衰减因子为$(1 + 0.9)^{100} = 1.9^{100} \approx 7.5 \times 10^{17}$——梯度被放大了！当然，实际网络中$\lambda$的值在不同层之间变化，有些层的$\lambda$可能大于1（导致梯度膨胀），有些可能远小于1（导致梯度消失）。但由于恒等项$\mathbf{I}$的存在，每一层的梯度传播至少保留一个单位的"基准流量"，使得优化器始终能够接收到来自损失函数的有效信号。正是这种"有保证的最小梯度流"使得ResNet的优化 landscape 远比标准深度网络平坦。

### 4.5 两种快捷连接：恒等映射 vs. 投影映射
...
预激活结构使得梯度在反向传播中经过残差块时只需经过加法节点和权重矩阵，不经过任何ReLU非线性——这保证了梯度的无损传播，对于训练超过1000层的极深网络至关重要。He等人使用预激活ResNet成功训练了1001层的网络，Top-1错误率达到28.0%（在CIFAR-10上）。
...
### 4.6 ResNet在ImageNet上的表现

ResNet-152在ILSVRC 2015上取得了Top-5错误率3.57%的成绩——首次在ImageNet上超越了人类水平（人类Top-5错误率约为5-10%）。这一里程碑事件证明了深度残差学习的强大威力，也标志着CNN架构研究进入了一个全新的阶段。

**ResNet的工程实现细节。** 原始ResNet-50/101/152的训练使用了以下参数：初始学习率0.1（对应batch size 256，分布在8块GPU上），动量0.9，权重衰减0.0001，总共训练约180个epoch（在ImageNet上约为此数量级），学习率在第30、60、90个epoch时乘以0.1衰减。值得注意的是，ResNet没有使用VGG的尺度抖动（scale jittering），而是使用了固定的短边为256像素的随机裁剪。这一简化的数据增强策略配合残差连接的有效优化，已经足以达到超越人类水平的性能。

在测试阶段，ResNet-152的推理使用了**多裁剪密集评估（multi-crop dense evaluation）**——将每张测试图像缩放到5个尺度（短边为256、288、320、352、384），在每个尺度上密集滑动裁剪224×224区域（步长为32像素），加上水平翻转，共约120个裁剪，取所有裁剪预测概率的平均。多裁剪融合将单裁剪的Top-5错误率从约5.6%降至3.57%，提升约2个百分点——这一提升幅度与VGG的多裁剪提升幅度相近，说明多裁剪融合是一种与架构无关的通用精度提升策略。
...
## 5. ResNet的理论解释：离散Hamilton-Jacobi-Bellman方程与神经ODE
...
这一洞见（E, 2017; Haber和Ruthotto, 2017）具有深远的理论意义：它将深度学习的"深度"参数化为积分步长，而非网络层数。步长$h$的选取需要权衡精度（小步长）与效率（大步长），这与数值分析中的经典权衡完全一致。

### 5.2 Hamilton-Jacobi-Bellman方程的离散化
...
在卷积层中，BN通常应用于每个输出通道。对于一个输出形状为$(C, H, W)$的卷积层（C个通道，每个通道是H×W的空间特征图），BN将每个通道的C×H×W个值视为一个独立的小批量（对空间维度求平均），独立计算该通道的$\mu_{\mathcal{B}}$和$\sigma_{\mathcal{B}}^2$，然后对该通道的所有空间位置应用相同的归一化变换。
...
$$
\frac{\text{Params}_{\text{dsep}}}{\text{Params}_{\text{std}}} = \frac{1}{512} + \frac{1}{9} \approx 0.123
$$

即深度可分离卷积仅使用标准卷积约12.3%的参数。当通道数更大时，压缩比进一步提高。对于3×3核、512通道，参数量从约236万降至约29万，压缩约8.1倍。
...
Howard等人（2017）的MobileNet进一步验证了这一结论。MobileNet主要由深度可分离卷积构成，在ImageNet上达到70.6%的Top-1准确率（仅用43M FLOPs和43万个乘法加法），相比同等计算预算下的标准卷积网络，精度损失极小但速度大幅提升。MobileNet v2（2018）通过引入倒残差结构（inverted residual）和线性瓶颈（linear bottleneck），进一步优化了深度可分离卷积的表达效率。
...
|------|------|------|------------|-----------|----------------|----------------|
| AlexNet | 2012 | 8 | 62.3 | 1.5 | 41.2 | 18.2 |
| VGG-16 | 2014 | 16 | 138 | 30.9 | 28.1 | 9.9 |
| VGG-19 | 2014 | 19 | 144 | 38.7 | 27.3 | 9.0 |
| GoogLeNet (v1) | 2014 | 22 | 6.6 | 3.8 | — | 6.67 |
| ResNet-18 | 2016 | 18 | 11.7 | 3.7 | 30.2 | 10.2 |
| ResNet-34 | 2016 | 34 | 21.8 | 7.3 | 26.7 | 8.7 |
| ResNet-50 | 2016 | 50 | 25.6 | 8.2 | 24.0 | 7.0 |
| ResNet-101 | 2016 | 101 | 44.6 | 15.5 | 22.4 | 6.2 |
| ResNet-152 | 2016 | 152 | 60.2 | 23.0 | 21.3 | 5.5 |
| ResNet-200 | 2016 | 200 | 71.3 | — | 21.2 | 5.4 |
| Inception-v3 | 2015 | 48 | 23.8 | 5.7 | — | 4.2 |
| Inception-v4 | 2016 | 70 | 42.7 | — | — | 3.0 |
| Inception-ResNet-v2 | 2016 | 96 | 55.8 | 13.2 | 19.9 | 4.9 |
| MobileNet-v1 | 2017 | 88 | 4.2 | 1.1 | 36.7 | 15.4 |
| MobileNet-v2 | 2018 | 88 | 3.5 | 0.6 | 28.0 | 9.4 |

*注：GoogLeNet和Inception系列使用多尺度裁剪和融合评估；带"—"表示原始论文未报告该指标或使用不同评估协议。AlexNet的FLOPs为约略值，不同实现略有差异。*

**训练超参数对比。** 各架构在ImageNet上的训练设置存在显著差异，这些差异对最终性能有不可忽视的影响。下表汇总了各论文报告的关键训练超参数：

| 架构 | 优化器 | 初始学习率 | Batch Size | Epochs | 学习率衰减策略 | 权重衰减 | 数据增强 |
|------|--------|-----------|------------|--------|--------------|---------|----------|
| AlexNet | SGD | 0.01 | 128 | 90 | 每30epoch×0.1 | 0.0005 | 随机裁剪+水平翻转 |
| VGG-16/19 | SGD | 0.01 (batch 256) | 256 | 370 | 指数衰减 | 0.0005 | 随机裁剪+水平翻转+颜色抖动 |
| GoogLeNet | SGD | 0.2 | 32×8=256 | — | 每8epoch×0.96 | 0.0002 | 随机裁剪+水平翻转+PCA颜色增强 |
| ResNet-50 | SGD | 0.1 | 256 | 180 | 每30epoch×0.1 | 0.0001 | 随机裁剪+水平翻转 |
| Inception-v3 | RMSProp | 0.045 | 128 | 500 | 每2.3epoch×0.94 | 0.00004 | 随机裁剪+水平翻转+颜色增强 |

值得注意的是，VGG-16/19的训练使用了高达370个epoch（而AlexNet仅90个epoch），且batch size为256（当时使用多GPU训练），这是VGG能够在如此深度的网络上收敛的关键条件之一。GoogLeNet使用了特殊的"每epoch学习率衰减0.96"策略（相当于大约每8个epoch衰减一半），配合较大的初始学习率0.2，在较小的batch size（32，分布在8块GPU上）下实现了稳定训练。
...
由于GoogLeNet和Inception系列使用了多裁剪融合评估，其报告的Top-5错误率（6.67%）与ResNet使用单裁剪的报告值（5.5%）之间并不存在直接的公平可比性。如果统一使用单裁剪评估，ResNet-152的Top-5错误率约为4.5-5.0%，与GoogLeNet的多裁剪结果更为接近。

### 9.3 数据分析与趋势解读

从表中可以观察到几个关键趋势：

**趋势一：深度与精度的正相关性（在残差连接出现之前）。** 在VGG和GoogLeNet时代，更深的网络确实倾向于取得更低的错误率——但边际收益递减明显。VGG-19比VGG-16深3层，Top-1错误率仅降低0.8个百分点，却增加了约30%的计算量。GoogLeNet在仅22层的情况下达到了与VGG-19相当的错误率（Top-5 6.67% vs. 9.0%），靠的是Inception模块的参数效率而非单纯的深度。值得注意的是，AlexNet仅有8层（含参数层），其Top-1错误率（41.2%）与VGG-16的28.1%之间存在超过13个百分点的差距——这说明从"浅层网络"到"深层网络"的跨越是最具决定性的单一步骤，而在此基础上的深度增加（16层→19层→50层→152层）带来的是渐进式的精细改进。

**趋势二：残差连接打破了深度-精度关系的瓶颈。** ResNet的出现使得深度与精度之间恢复了稳健的正相关关系。ResNet-50（50层）的错误率（Top-1 24.0%）已经优于VGG-19的27.3%，而参数量仅为VGG-19的17.8%。ResNet-152的Top-1错误率降至21.3%，首次低于人类水平（约24% Top-1）。特别值得注意的是ResNet-152与Inception-ResNet-v2的比较：两者深度相近（152 vs. 96），参数量相近（60M vs. 56M），但ResNet-152的Top-1错误率（21.3%）优于Inception-ResNet-v2的19.9%——实际上Inception-ResNet-v2的19.9%优于ResNet-152，说明Inception模块和残差连接的结合比单独使用残差连接更有效。这一结果直接推动了后续Inception-ResNet系列和SENet（Hu等人，2018）的发展。

**趋势三：参数效率的代际跃迁。** GoogLeNet以6.6M参数达到与VGG-16（138M参数）相近的错误率，参数效率提升了约20倍。ResNet系列延续了这一趋势——ResNet-50用25.6M参数超过了VGG-19的144M参数。这一趋势在MobileNet中达到顶峰——仅用4.2M参数和1.1G FLOPs，MobileNet-v1就实现了可接受的精度，为移动端部署铺平了道路。从VGG到MobileNet，参数效率提升了约33倍（138M → 4.2M），而精度损失在可控范围内——Top-5错误率从9.0%（VGG-19）到15.4%（MobileNet-v1），差距约6.4个百分点，但FLOPs从38.7G降至1.1G，压缩比约35倍。

**趋势四：深度可分离卷积的极端参数效率。** MobileNet-v1仅1.1G FLOPs，不到VGG-16（30.9G）的4%，在移动设备上实现了实时推理。MobileNet-v2进一步将FLOPs降至0.6G，同时保持了可接受的精度（Top-1 28.0%），证明了深度可分离卷积在极端资源约束下的可行性。需要指出的是，MobileNet系列的低错误率（MobileNet-v1的Top-1 36.7%）部分来自于更大的输入分辨率（224×224）和更长的训练时间——如果统一使用相同的训练协议，MobileNet与标准卷积网络的精度差距会进一步缩小。

**从单一指标到综合评估。** 表格中的Top-1/Top-5错误率只是评估网络性能的一个维度。实际部署中还需要考虑：推理延迟（latency）、内存占用（model size）、能耗（energy consumption，对移动端尤为重要）和可训练性（training stability）。例如，ResNet-152虽然精度最高，但其23G FLOPs和60M参数在实时应用中不可接受；VGG-19的38.7G FLOPs中大部分来自全连接层——如果去掉全连接层（如使用全局平均池化），VGG的计算量可以大幅降低，精度损失极小。这些"纸上指标"与"实际性能"之间的差距，正是架构工程师需要在实际部署中解决的核心问题。
...
| 架构 | 模型大小（MB） | 推理时间（ms/image） | FLOPs（G） | 精度-速度权衡比 |
|------|--------------|-------------------|-----------|--------------|
| AlexNet | 233 | 1.4 | 1.5 | 基准 |
| VGG-16 | 528 | 4.2 | 30.9 | 低（大而慢） |
| GoogLeNet | 27 | 2.8 | 3.8 | 高 |
| ResNet-50 | 98 | 4.1 | 8.2 | 中 |
| ResNet-152 | 230 | 9.1 | 23.0 | 低 |
| MobileNet-v1 | 17 | 3.2 | 1.1 | 极高 |

从表中可以看出，GoogLeNet在精度和速度之间取得了最好的平衡——仅27MB模型大小和2.8ms推理时间，Top-5错误率仅为6.67%，优于VGG-16的9.9%（528MB，4.2ms）。ResNet-50虽然精度更高（Top-5 7.0%），但98MB模型大小和4.1ms推理时间在资源受限场景中不够理想。MobileNet-v1以17MB和3.2ms实现了移动端实时推理，证明了极低计算预算下依然可以保持可接受的精度。
...
**模型压缩技术。** 除了架构层面的参数效率优化外，模型压缩技术（量化、剪枝、知识蒸馏）是降低部署成本的另一条重要途径。量化将32位浮点权重替换为8位整数甚至更低精度，模型大小可压缩4-8倍，推理速度提升2-4倍，精度损失通常控制在1%以内。Han等人（2015）的深度压缩（deep compression） pipeline 结合剪枝、量化和霍夫曼编码，将AlexNet的模型大小从233MB压缩至6.9MB（压缩比约34倍），精度损失几乎为零。知识蒸馏（Hinton等人，2015）通过让学生网络模仿教师网络的软标签（soft labels），使得小模型可以继承大模型的知识——例如，用ResNet-152作为教师网络训练MobileNet，可以将MobileNet的Top-1错误率从36.7%降至约30%，在不改变架构的前提下大幅提升了小模型的精度。
...
- **颜色增强：** GoogLeNet使用了PCA颜色增强（PCA jittering）——对RGB通道进行PCA，在主方向上按一定幅度添加随机扰动。VGG-16/19使用了更简单的随机颜色抖动（随机调整亮度、对比度、饱和度）。
...
**集成学习（Ensemble Learning）与黄金时代架构。** 在ILSVRC竞赛中，集成多个模型的预测结果是提升精度的标准策略。2015年ILSVRC的获胜方案是16个ResNet-152模型的集成，将Top-5错误率降至3.57%。集成策略包括：（1）多裁剪集成——对同一图像的不同裁剪位置和尺度分别预测后取平均；（2）多模型集成——将不同架构（如ResNet-152 + Inception-ResNet-v2 + ResNet-101）的预测概率取平均；（3）测试时增强（test-time augmentation, TTA）——在推理时对输入应用随机变换（翻转、裁剪、色彩抖动），对多个增强版本的输出取平均。多裁剪集成本身可以将单模型的Top-5错误率降低约1-2个百分点，而多模型集成可以进一步降低0.5-1个百分点。ILSVRC 2016的获胜方案通过集成多个模型，将Top-5错误率降至约2.99%（不使用外部数据），证明了集成学习在ImageNet尺度上的有效性。然而，集成策略的成本是推理延迟和模型存储的成倍增加——16个ResNet-152模型的推理时间是单模型的16倍，总内存占用约3.7GB，在真实部署中不可接受。

**对抗鲁棒性与架构选择。** 黄金时代的架构在对抗样本（adversarial examples）面前表现出不同的鲁棒性。Goodfellow等人（2015）发现，VGG-16和GoogLeNet对FGSM（Fast Gradient Sign Method）攻击的鲁棒性相近——Top-1错误率从正常的28.1%（VGG-16）升至攻击下的约85%。ResNet由于使用了更多的非线性层（瓶颈结构中的三层堆叠），在某些攻击下的鲁棒性略优于VGG，但差距不大。这一观察表明：架构设计主要影响"干净精度"（clean accuracy），而对抗鲁棒性更多地取决于训练策略（对抗训练、数据增强）而非架构本身。Szegedy等人（2014）在GoogLeNet论文中首次报告了对抗样本对Inception架构的影响——他们发现，即使使用多尺度裁剪和融合，GoogLeNet仍会被微小扰动（人眼不可见的像素级修改）欺骗，输出完全错误的分类结果。这一发现引发了深度学习安全领域的广泛关注。
