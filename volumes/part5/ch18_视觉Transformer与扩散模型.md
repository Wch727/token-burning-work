...
$$
\mathbf{z}_0 = \left[ \mathbf{x}_p^1 \mathbf{E};\, \mathbf{x}_p^2 \mathbf{E};\, \cdots;\, \mathbf{x}_p^N \mathbf{E} \right] + \mathbf{E}_{\text{pos}} \in \mathbb{R}^{N \times D}
$$

这里 $\mathbf{z}_0 \in \mathbb{R}^{N \times D}$ 是Transformer编码器的输入序列，$\mathbf{E}_{\text{pos}} \in \mathbb{R}^{N \times D}$ 为位置编码（详见6.2.2节），分号表示序列维度上的拼接操作（concatenation along the sequence dimension）。
...
$$
\begin{aligned}
\mathbf{z}'_\ell &= \text{MHSA}(\text{LN}(\mathbf{z}_{\ell-1}), \mathbf{z}_{\ell-1}) + \mathbf{z}_{\ell-1} \\
\mathbf{z}_\ell &= \text{MLP}(\text{LN}(\mathbf{z}'_\ell), \mathbf{z}'_\ell) + \mathbf{z}'_\ell, \quad \ell = 1, \ldots, L
\end{aligned}
$$
...
**小规模数据集上的表现：** 当在ImageNet-1K（约120万张训练图像，1000个类别）上从头训练时，ViT的表现不及同等计算预算下的CNN基线（如ResNet）。具体而言，ViT-B/16（Base模型，12层Transformer编码器，隐藏维度768，12个注意力头，patch大小16×16，约86M参数）在ImageNet-1K上仅达到约78-79%的top-1准确率，而ResNet-50（约25M参数）可以达到约80%以上的top-1准确率，ResNet-152（约60M参数）更是达到约82%。这一差距的根本原因在于：CNN的归纳偏置（局部连接性和平移等变性）在小数据集上起到了正则化作用，防止了模型过拟合——这些归纳偏置大大缩减了假设空间（hypothesis space）的规模，使得模型在有限数据下就能学到有效的表征。相比之下，ViT缺乏这些归纳偏置，拥有更大的假设空间，需要更多的数据才能将搜索空间约束到正确的区域。
...
**大规模数据集上的突破：** 当训练数据扩展到JFT-300M（包含约3亿张图像的大规模Google私有数据集，涵盖约18K类别）时，ViT的表现发生了质变。ViT-L/16（Large模型，24层Transformer编码器，隐藏维度1024，16个注意力头，约304M参数）在ImageNet-1K上达到了88.35%的top-1准确率，优于此前最优的CNN模型（包括使用大量数据增强和自训练的Noisy Student等）。ViT-H/14（Huge模型，32层，隐藏维度1280，16个注意力头，约632M参数）更是将准确率推高至88.55%，将ImageNet-top-1准确率的纪录向前推进了一大步。
...
为了更清晰地理解ViT相对于传统CNN的优势与劣势，表6-1从多个维度对两者进行了系统性比较。这一比较涵盖了架构设计、计算特性、数据效率、泛化能力和下游任务适用性等核心维度。
...
## 6.3 DeiT：数据高效的Vision Transformer

### 6.3.1 训练策略与数据增强

Data-efficient Image Transformer（DeiT）（Touvron et al., 2020）旨在解决ViT的两个核心痛点：对大规模标注数据的极度依赖和较慢的收敛速度。DeiT的核心洞察是：通过精心设计的训练策略，可以在ImageNet alone（不使用JFT-300M等大规模数据集）上将ViT的性能提升到与CNN相当的水平。
...
其中 $\mathbf{z}_{\text{patch}_j}^{(i-1)}$ 表示第 $i-1$ 阶段中一个 $2 \times 2$ 邻域内的第 $j$ 个patch的特征向量，Concat操作沿通道维度进行拼接，Linear层将拼接后的 $4 \times 2C$ 维向量投影到 $2C$ 维。
...
在目标检测和实例分割等下游任务中，Swin Transformer的Stage 2至Stage 4输出直接作为特征金字塔网络（FPN）或路径聚合网络（PANet, Liu et al., 2018）的输入。例如，在Mask R-CNN（He et al., 2017）框架中，Swin Transformer作为骨干网络（Backbone）替换原来的ResNet-50/101，将COCO目标检测的框AP从42.0%（ResNet-50）提升到51.1%（Swin-T），将实例分割的mask AP从37.8%提升到48.1%。更大的Swin-S和Swin-B模型分别达到了53.9%框AP/51.6% mask AP和53.2%框AP/52.1% mask AP，全面超越了使用ResNeXt-101-64x4d骨干的最强CNN基线。

从理论视角审视，Swin Transformer的成功揭示了视觉任务中一个深刻的设计权衡：归纳偏置的双刃剑效应。CNN的局部连接性和平移等变性在数据有限时是宝贵的先验知识，但在大规模数据时代，过于强烈的归纳偏置反而限制了模型的表征能力上限。Swin Transformer通过局部窗口注意力保留了部分归纳偏置（ locality），同时通过移位窗口实现了跨区域信息交互，在效率和性能之间取得了最佳平衡。事实上，Swin Transformer的计算复杂度为：
...
He等人的实验表明，在ImageNet-1K上使用ViT-Large进行MAE预训练（无需标签）后，仅用微调（Fine-tuning）即可达到87.8%的top-1准确率，这一结果优于同期所有需要标签的监督预训练方法。更重要的是，MAE的预训练在更大的数据集（如ImageNet-21K）上展现出了更强的可扩展性（scalability）——模型越大、数据越多，MAE的收益越明显，这与BERT在NLP领域的观察一致。
...
### 6.6.2 反向去噪过程：Score Matching与DDPM
...
### 6.6.4 条件生成：Classifier Guidance与Classifier-Free Guidance
...
$$
p_{\text{guided}}(\mathbf{x} | \mathbf{c}) \propto \frac{p(\mathbf{x} | \mathbf{c})^w}{p(\mathbf{x})} = \frac{p(\mathbf{c} | \mathbf{x})^w \cdot p(\mathbf{x})^{1-w}}{Z(\mathbf{c})}
$$

当 $w > 1$ 时，分布被尖锐化（sharpened），更集中于条件 $\mathbf{c}$ 所指示的区域；当 $w < 1$ 时，分布被平坦化（flattened），增加了多样性。

### 6.6.5 Score Matching与随机微分方程（SDE）视角
...
### 6.7.2 Stable Diffusion与潜在扩散模型

Stable Diffusion（Rombach et al., 2022）是LAION团队（Stability AI）于2022年8月发布的开源文本到图像生成系统。与DALL-E 2在像素空间运行扩散模型不同，Stable Diffusion基于潜在扩散模型（Latent Diffusion Model, LDM）架构，在压缩的潜在空间而非像素空间中进行扩散和去噪，这使其能够在消费级GPU上高效运行，成为AI图像生成民主化的关键技术推动者。
...
$$
\mathcal{L}_{\text{VAE}} = \mathbb{E}_{\mathbf{x}} \left[ \|\mathbf{x} - \mathcal{D}(\mathcal{E}(\mathbf{x}))\|^2 + \frac{1}{2} \left( \|\boldsymbol{\mu}_{\mathcal{E}}(\mathbf{x})\|^2 + \|\boldsymbol{\sigma}_{\mathcal{E}}(\mathbf{x})\|^2 - \log(\|\boldsymbol{\sigma}_{\mathcal{E}}(\mathbf{x})\|^2) - 1 \right) \right]
$$

其中 $\boldsymbol{\mu}_{\mathcal{E}}$ 和 $\boldsymbol{\sigma}_{\mathcal{E}}$ 分别为VAE编码器输出的均值和对数方差。为了减少重建模糊，LDM使用了 perceptual loss 和 patch-based adversarial loss（GAN判别器），这在原始论文中被称为LPIPS（Zhang et al., 2018）加权重建。
...
### 6.7.4 Google Imagen：级联扩散与T5语言理解

Google Research在2022年发表的Imagen（Saharia et al., 2022）代表了文本到图像生成的另一条技术路线。与DALL-E 2依赖CLIP进行文本-图像语义对齐不同，Imagen的核心设计哲学是：大规模语言模型的文本理解能力是文本到图像生成质量的关键瓶颈——一个强大的语言理解 backbone 比增大图像生成模型或提高文本-图像对齐的粒度更能显著提升生成结果的质量。

基于这一洞察，Imagen使用了一个预训练的T5-XXL语言模型（约4.6B参数，Raffel et al., 2020）作为文本编码器，将输入文本prompt映射为语义丰富的嵌入序列。T5-XXL在C4（约750GB文本）上进行过预训练，具有强大的语言理解和常识推理能力。实验表明，将文本编码器从CLIP-B/32（仅约400M参数）升级到T5-XXL（约4.6B参数）将FID从7.27提升到3.45——这一提升幅度远大于任何其他架构修改（如增加U-Net深度、使用更高分辨率的潜在空间等）。
...
每个超分辨率模型都使用了无分类器引导（CFG），引导强度 $w$ 在基础模型中取7.0，在两个超分辨率模型中取2.0（较低的引导强度以保持细节多样性）。Imagen在COCO数据集上达到了7.27的FID（无引导）和4.0的FID（有引导），在人类评估中显著优于同期DALL-E 2和其他基线。
...
**架构设计：** ControlNet的核心架构包含两个分支：(a) **锁定分支（Locked Branch）**：冻结的原始扩散模型U-Net，保留其从大规模预训练中学到的通用生成能力；(b) **控制分支（Control Branch）**：一个可学习的副本，其结构与U-Net的编码器部分完全镜像（mirror），包含12个可训练的ControlNet块（每个块对应U-Net的一个残差块或注意力块）。
...
**训练策略：** ControlNet采用了一个高效的微调策略。首先，将一个小型数据集（如COCO或内部数据集，包含约数万到数十万张图像及其对应的条件标注）上的扩散模型进行端到端微调，其中只有ControlNet的参数被更新（U-Net参数冻结）。其次，使用小的batch size（如4-16）和学习率（如 $10^{-5}$），因为ControlNet的参数规模远小于完整U-Net（约1.5B可训练参数 vs. 约2.3B总参数）。训练使用标准的扩散损失（式6-35），仅在被掩码的干净图像和对应的条件图之间计算。
...
ControlNet的发布被认为是文本到图像生成领域的"基础设施级"突破。截至2024年初，ControlNet已被应用于 countless 的创意工作流中，并与Stable Diffusion、DALL-E等系统广泛集成。其技术思想——通过一个可训练的"控制副本"将外部条件注入预训练的生成模型——也被扩展到视频生成（ControlNet-Video）、3D生成（ControlNet-3D）等新领域。

## 6.8 视频理解：时空表征学习

视频理解是计算机视觉中最复杂、最具挑战性的子领域之一。与静态图像不同，视频同时包含空间（appearance）和时间（motion）两个维度的信息，需要对它们进行有效的联合建模。近年来，随着Transformer架构在视觉领域的成功，视频理解领域也经历了一系列深刻的技术革新。

### 6.8.1 TimeSformer：时空注意力的可分分解
...
1. **随机掩码（Random Masking）：** 随机选择一定比例的tubelet进行掩码，类似于图像MAE。但VideoMAE发现，对于视频，过高的掩码比例（如图像中的75%）会导致重建任务过于简单（因为相邻帧的高度相似性提供了丰富的时间冗余信息）。
...
$$
\mathcal{L}_{\text{VideoMAE}} = \frac{1}{N_{\text{mask}}} \sum_{i \in \mathcal{M}} \| \text{HOG}(\mathbf{v}_i) - \text{Decoder}(\mathbf{z}_{\text{mask}, i}) \|^2
$$
...
### 6.9.2 Fréchet Inception Distance（FID）
...
**密度和覆盖度（Density and Coverage）：** Naik & Rauschmayr（2021）提出了改进的Density和Coverage指标，使用k近邻（k-NN）来估计流形的局部密度，解决了原始Precision/Recall在密度不均匀流形上的偏差问题。Density衡量生成点周围真实点的局部密度（反映生成质量的局部集中度），Coverage衡量真实流形被生成点覆盖的比例。

### 6.9.4 Sliced Fréchet Inception Distance（sFID）

标准FID使用完整的2048维Inception特征，但高维特征空间中的协方差矩阵估计可能面临"维数灾难"——当特征维度远大于样本量时，协方差矩阵估计不准确，FID值对样本量敏感。Sliced FID（sFID, Nielson et al., 2020）通过将高维特征投影到多个随机一维子空间上来缓解这一问题。

具体地，sFID首先将2048维的Inception特征通过 $L$ 个随机投影矩阵 $\{\mathbf{R}_\ell\}_{\ell=1}^L$ 投影到一维空间（每个 $\mathbf{R}_\ell \in \mathbb{R}^{2048 \times 1}$），然后对每个一维投影分别计算Fréchet距离，最后取平均：

$$
\text{sFID} = \frac{1}{L} \sum_{\ell=1}^L \text{FID}(\mathbf{x} \mathbf{R}_\ell, \mathbf{y} \mathbf{R}_\ell) = \frac{1}{L} \sum_{\ell=1}^L \left( \|\boldsymbol{\mu}_{\text{real}} \mathbf{R}_\ell - \boldsymbol{\mu}_{\text{gen}} \mathbf{R}_\ell\|^2 + \text{Var}(\mathbf{x} \mathbf{R}_\ell) + \text{Var}(\mathbf{y} \mathbf{R}_\ell) - 2\sqrt{\text{Var}(\mathbf{x} \mathbf{R}_\ell) \cdot \text{Var}(\mathbf{y} \mathbf{R}_\ell)} \right)
$$

其中 $\text{Var}(\cdot)$ 为一维投影后的方差。sFID的理论优势在于：一维高斯分布的Fréchet距离有极为简洁的闭式解，不需要矩阵平方根计算；多个随机投影的 averaging 提供了对高维分布差异的稳健估计，减少了对单个方向的敏感性。在实践中，sFID通常比标准FID对样本量更鲁棒——当生成样本量较少（如每类仅50张图像）时，sFID的估计方差显著小于FID。
...
1. **同时评估真实性和多样性：** IS仅评估生成图像在类别上的清晰度和多样性（通过边缘分布），但不评估生成分布与真实分布的整体匹配度。FID通过比较两个完整分布，同时捕捉了真实性和多样性。
2. **对模型崩溃更敏感：** 当生成模型发生模式崩溃（mode collapse）时，$P_{\text{gen}}$ 的协方差矩阵 $\boldsymbol{\Sigma}_{\text{gen}}$ 会退化（秩降低），导致FID显著增大。
3. **与人类判断更一致：** 大量实证研究表明，FID与人类对生成质量的主观判断具有较高的Spearman相关性。

FID的局限性包括：需要大量真实图像（通常建议至少10000张）来准确估计 $\boldsymbol{\mu}_{\text{real}}$ 和 $\boldsymbol{\Sigma}_{\text{real}}$；对特征提取器的选择敏感（不同架构或不同层的Inception网络会产生不同的FID值）；对于细粒度类别差异不够敏感（因为Inception-v3的中间特征主要编码高层语义而非细粒度纹理）。

### 6.9.2 Inception Score（IS）

Inception Score（IS, Salimans et al., 2016）是较早被广泛采用的生成模型评估指标，其设计基于两个直觉：好的生成图像应该具有清晰可辨的类别（高置信度），且生成模型应该产生多样化的类别（高熵）。

具体地，给定一个预训练的Inception-v3分类器，对于 $N$ 张生成图像，提取每个图像的类别条件分布 $p(y | \mathbf{x})$（即Softmax输出）。IS定义为：

$$
\text{IS} = \exp\left( \mathbb{E}_{\mathbf{x} \sim P_{\text{gen}}} \left[ D_{\text{KL}}(p(y | \mathbf{x}) \| p(y)) \right] \right) = \exp\left( \frac{1}{N} \sum_{i=1}^N \sum_{y=1}^K p(y | \mathbf{x}_i) \log \frac{p(y | \mathbf{x}_i)}{p(y)} \right)
$$

其中 $p(y) = \mathbb{E}_{\mathbf{x}}[p(y | \mathbf{x})]$ 为生成的边缘类别分布，$D_{\text{KL}}$ 为KL散度。

IS的数学含义可以从信息论角度理解。KL散度 $D_{\text{KL}}(p(y | \mathbf{x}) \| p(y))$ 衡量了条件分布 $p(y | \mathbf{x})$ 与边缘分布 $p(y)$ 之间的差异。当每张生成图像都被分类器以高置信度分类到某一类别（即 $p(y | \mathbf{x})$ 接近独热分布），且不同图像覆盖了多样化的类别（即 $p(y)$ 接近均匀分布）时，KL散度最大化，IS也最大化。

IS的局限性包括：

1. **无法评估与真实分布的匹配度：** IS不涉及真实图像分布，因此生成模型可能通过在少数类别上生成高质量图像而获得高分，同时完全遗漏其他类别。
2. **对类别不平衡敏感：** 如果测试数据集本身类别不平衡，IS的解释会变得模糊。
3. **无法检测模式崩溃：** 如果模型仅在少数类别上生成图像但质量很高，IS可能仍然很高。
4. **依赖预训练分类器：** IS的值取决于特定Inception-v3模型的分类能力，不同模型版本会产生不同的分数。

在实践中，IS通常与FID配合使用，前者提供类别级别的质量评估，后者提供分布级别的整体评估。

### 6.9.3 CLIP Score

CLIP Score（Ramesh et al., 2022; Hessel et al., 2021）是基于CLIP模型的文本-图像对齐评估指标，专门用于衡量生成图像与文本描述之间的语义一致性。在文本到图像生成任务中，FID和IS评估的是生成图像的整体质量（相对于训练分布），但不直接评估生成图像是否忠实地反映了文本prompt的语义内容。CLIP Score填补了这一空白。
...
其中 $f_{\text{text}}$ 和 $f_{\text{image}}$ 分别为CLIP的文本编码器和图像编码器，$\langle \cdot, \cdot \rangle$ 为余弦相似度，乘以100是为了将分数映射到一个更直观的范围。CLIP Score的取值范围通常在0到100之间（在归一化后的嵌入空间中），分数越高表示生成图像与文本描述的语义一致性越好。

对于大规模评估，给定一个包含 $N$ 个（文本，图像）对的测试集，报告平均CLIP Score（平均余弦相似度）：
...
1. **开放词汇评估：** 由于CLIP在4亿个（图像，文本）对上进行了预训练，它能够理解任意自然语言描述，评估指标不局限于预定义类别。
2. **语义级评估：** CLIP Score衡量的是语义对齐而非像素级匹配——即使生成图像在像素上与参考图像不同，只要语义内容一致，CLIP Score仍可很高。
3. **无需参考图像：** 对于文本到图像任务，通常没有"标准答案"参考图像，CLIP Score仅需文本prompt即可进行评估。

CLIP Score的局限性包括：

1. **与人类判断的偏差：** CLIP Score与人类对生成质量的主观判断并不完全一致。有时，生成图像可能与文本具有高CLIP Score但存在明显的视觉伪影或逻辑错误。
2. **对表面特征的偏好：** CLIP可能对图像中的表面特征（颜色、纹理、物体存在性）比对复杂关系（空间布局、计数、动作）赋予更高的权重。
3. **模态瓶颈：** CLIP的嵌入空间是预训练时固定的，它可能无法捕捉到后续生成模型的新能力或新的视觉概念。

在实践中，全面的评估通常采用FID + CLIP Score的组合：FID衡量生成图像的保真度和多样性（相对于训练分布），CLIP Score衡量生成图像与文本提示的语义对齐度。这两个指标从不同角度提供了对生成模型性能的刻画。

### 6.9.6 生成模型的系统比较：扩散模型、GAN与自回归模型
